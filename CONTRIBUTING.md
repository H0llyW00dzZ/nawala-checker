...
*   If your pull request adds a new feature, changes the public API, or modifies existing behavior, you **must synchronize all documentation sources** to ensure technical accuracy and consistency across all documentation sources.

### Synchronizing Documentation

> [!NOTE]
> This process is only applicable when human users are using opencode.ai. If not, follow the manual steps below.

When making changes that affect documentation, run the `sync-docs` command via opencode.ai to ensure consistency across multilingual documentation, GoDoc, examples, and CLI usage text.
...
1. **Identify Changes**: Review your code changes to determine what documentation needs updating (new configuration options, changed function signatures, new CLI commands or flags, modified behavior or error conditions).

2. **Update README Files**: Update both language versions (`README.md` and `README.id.md`).

3. **Update GoDoc**: Ensure package documentation reflects new/changed APIs in `src/nawala/docs.go` and `internal/cli/docs.go`.

4. **Update Examples**: Add or modify examples in `examples/` directory to demonstrate new features.

5. **Update CLI Usage**: For CLI changes, update embedded usage text in `internal/cli/usage/`.

6. **Verify Consistency**: Run tests to ensure all documentation sources are technically accurate:
   ```bash
   make test-verbose
   ```
